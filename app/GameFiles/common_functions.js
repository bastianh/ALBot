...
var bank_packs={
    ...
    "items36":["bank_u",4075000000,1450],
    "items37":["bank_u",4075000000,1450],
    "items38":["bank_u",4075000000,1450],
    "items39":["bank_u",4075000000,1450],
    "items40":["bank_u",4075000000,1650],
    "items41":["bank_u",4075000000,1650],
    "items42":["bank_u",4075000000,1650],
    "items43":["bank_u",4075000000,1650],
    "items44":["bank_u",4075000000,1650],
    "items45":["bank_u",4075000000,1850],
    "items46":["bank_u",4075000000,1850],
    "items47":["bank_u",4075000000,1850],
};
...
function resolve_deferred(name,data)
{
    ...
    if(current_deferred.start) push_ping(mssince(current_deferred.start));
    try{
...
function calculate_item_value(item,m)
{
    ...
    var def=G.items[item.name],value=def.cash&&def.g||def.g*(m||0.6),divide=1; // previously 0.8
...
function calculate_item_properties(def,item)
{
    ...
    for(p in prop)
        if(!in_arr(p,["evasion","miss","reflection","dreturn","lifesteal","manasteal","attr0","attr1","crit","critdamage","set","class"])) prop[p]=round(prop[p]);
