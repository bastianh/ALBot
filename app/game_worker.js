...
process.on('uncaughtException', function (exception) {
    ...
    // parentPort.postMessage({type: "status", status: "error"});
    // process.exit(-1);
});
...
Game.prototype.init = function () {
    ...
    parentPort.postMessage({type: "status", status: "initialized"});
    ...
    socket.on("disconnect", function () {
        ...
        parentPort.postMessage({type: "status", status: "disconnected"});
        self.stop();
    });
...
async function main() {
    try {
        let httpWrapper = new HttpWrapper(workerData.auth);
        let gameData;
        let success = false;
        while (!success) {
            try {
        ...
        let game = new Game(workerData.ip, workerData.port, workerData.name, workerData.script, "", gameData, httpWrapper, X);
        game.init();
    } catch (e) {
